...
import { faHome, faCar, faUser, faCog, faInfoCircle
...
function SideBarCustom( {isOnMap}  : {isOnMap?: boolean}) {
  ...
  return (
    <div>
      ...
      <Sidebar visible={isSidebarOpen} onHide={toggleSidebar} className="p-sidebar-md" style={{ width: "280px" }}>
          ...
          <div className="overflow-y-auto mt-4">
            <ul className="list-none p-3 m-0">
              {inviteRoutes.map((item, index) => (
                <li key={index} onClick={() => { navigate(item.route); toggleSidebar(); }}>
