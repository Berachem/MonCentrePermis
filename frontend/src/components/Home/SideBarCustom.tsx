...
// No need for callbacks anymore since we're handling the modal directly
interface SideBarCustomProps {
  // Any props needed from parent
}

function SideBarCustom({}: SideBarCustomProps) {
  ...
  const [profileIdRequested, setProfileIdRequested] = useState<string>("");
  ...
  const openProfileModal = (id?: string) => {
    setProfileIdRequested(id || userId || "");
    setProfileModalVisible(true);
    toggleSidebar(); // Close the sidebar when opening the modal
  };
  ...
  // Function to handle user's profile click
  ...
  return (
    <>
                ...
                {process.env.NODE_ENV === "development" && (
                  ...
                  >
                    <a className="flex items-center p-2 text-gray-700 hover:bg-gray-100 rounded-lg transition-colors duration-150 w-full">
                      ...
                      <span className="font-medium">Voir profil test (ID 4)</span>
                      <Ripple />
                    </a>
                  </li>
                )}
      ...
      {/* ProfileModal directly inside SideBarCustom */}
      <ProfileModal
